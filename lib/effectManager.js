...
var EffectManager = class EffectManager {
    ...
    // ── Chrome actors (Side docks, right-dock, Dash) ─────────────────────────
    ...
    _getChromeActors() {
        ...
        for (const item of tracked) {
            ...
            if (actor && actor !== Main.panel)
                actors.add(actor);
        }

        // 2. Scan uiGroup children for custom extension panels (e.g. right-dock)
        if (Main.uiGroup) {
            for (const child of Main.uiGroup.get_children()) {
                if (!child || child === Main.panel || child === Main.layoutManager._backgroundGroup)
                    continue;

                const cls = (child.style_class || (typeof child.get_style_class_name === 'function' ? child.get_style_class_name() : '')) || '';
                if (cls.includes('dock') || cls.includes('dash') || cls.includes('panel')) {
                    actors.add(child);
                }
            }
        }
        ...
        for (const actor of currentChromeActors) {
            if (!this._chromeEffects.has(actor) && !actor.get_effect(EFFECT_NAME)) {
                try {
                    ...
                    console.error('[Pasynkov Tint] attach to chrome actor failed:', e);
                }
            }
        }

        for (const [actor, effect] of this._chromeEffects) {
            if (!currentChromeActors.has(actor)) {
                try { actor.remove_effect_by_name(EFFECT_NAME); } catch (_) {}
