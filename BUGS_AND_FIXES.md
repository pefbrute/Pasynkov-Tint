...
## 🟡 БАГ 9 — При переключении через Alt+Tab на всплывающую панель переключения иконки не тонируются

**Когда:** Включён per-window режим. Нажимаешь Alt+Tab для переключения между окнами.

**Симптом:**
Всплывающая панель переключения приложений Alt+Tab (`AppSwitcherPopup` / `switcher-list`) отображается в цветах без фильтра.

**Причина:**
Панель Alt+Tab создается динамо-актором `SwitcherPopup` при нажатии комбинации клавиш и помещается в оверлей `Main.uiGroup` или `Main.layoutManager.modalDialogGroup`, минуя список окон приложений `MetaWindowActor`.

**Решение (в процессе реализации):**
Добавить маску `'switcher'` и `'modal'` в селектор акторов `_getChromeActors()`, а также опрашивать появление модальных окон при нажатии Alt+Tab / изменении модального состояния.
